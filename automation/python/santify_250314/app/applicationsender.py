...
install_appPkg = "luna-send -i 1 -f luna://com.webos.service.pkgInstall/install"
...
class ApplicationSender:
    ...
    def send_application(self, application):
        ...
        self.log(f"Sending to app home: {apphome_result['returnValue']}")
        self.wait(5)
        self.log(f"Sending application: {application}")
        app_result = self.do_send_app(application)
        self.log(f"Sending to application: {app_result['returnValue']}")
        self.wait(5)
