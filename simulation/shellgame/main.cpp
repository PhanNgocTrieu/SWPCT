/*
    Links: https://usaco.org/index.php?page=viewproblem2&cpid=891
*/
...
void solve() {
    ...
    loop(n) {
        ...
        swap(shell_gues[s_a], shell_gues[s_b]);
        counter[shell_gues[guess]]++;

        debugv(shell_gues);
        debugv(counter)
    }
    ans = max(counter[0], max(counter[1], counter[2]));
    cout << ans << endl;
}
