...
void solve() {
    ...
    loop(N) {
        ll in, out;
        cin >> in >> out;
        events.push_back(std::make_tuple(in, i, true));
        events.push_back(std::make_tuple(out,i, false));
    }
...
#ifdef READ_FILE
    FILE* f_in = freopen("lifeguards.in", "r", stdin);
    FILE* f_out = freopen("lifeguards.out", "w", stdout);
#endif
